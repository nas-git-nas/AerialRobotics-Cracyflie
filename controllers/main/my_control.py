...
class MyController():
    ...
    def step_control(self, sensor_data):
        ...
        if self._state == "takeoff":
            ...
            command = self._move(sensor_data)
        elif self._state == "search":
            command = self._search(sensor_data)
    ...
    def _move(self, sensor_data):
        # command = [0.0, 0.0, 0.0, self._height_desired]
        # self._state = "search"
        # print("_move: move -> search")
        # return command

        command = self._move2point(sensor_data, self._points[self._points_idx])
    ...
    def _search(self, sensor_data):
        ...
        # land if landing platform is reached
        if sensor_data['range_down'] < 0.42:
            control_command = [0.0, 0.0, 0.0, self._height_desired]
            self._state = "land"
            ...
            control_command = self._move2point(sensor_data, self._points[self._points_idx])

        # check if point is reached and increase point index if this is the case
        self._checkPoint(sensor_data=sensor_data, dist=0.05)

        self._checkForObstacles(sensor_data=sensor_data, command=control_command)

        return self._scaleCommand(command=control_command, scale=0.2)
